...
void BrowserView::setupUI() {
    ...
    backAction = toolbar->addAction(QIcon::fromTheme("go-previous", style()->standardIcon(QStyle::SP_ArrowBack)), "Back");
    forwardAction = toolbar->addAction(QIcon::fromTheme("go-next", style()->standardIcon(QStyle::SP_ArrowForward)), "Forward");
    refreshAction = toolbar->addAction(QIcon::fromTheme("view-refresh", style()->standardIcon(QStyle::SP_BrowserReload)), "Refresh");
    ...
    menuButton->setIcon(QIcon::fromTheme("application-menu", style()->standardIcon(QStyle::SP_TitleBarMenuButton)));
    ...
    // Connect signals
    connect(addressBar, &QLineEdit::returnPressed, this, &BrowserView::navigateToAddress);
    connect(webView, &QWebEngineView::urlChanged, this, &BrowserView::handleUrlChange);
    connect(webView, &QWebEngineView::loadProgress, this, &BrowserView::handleLoadProgress);
    connect(webView, &QWebEngineView::loadFinished, this, &BrowserView::handleLoadFinished);
    connect(webView, &QWebEngineView::customContextMenuRequested,
            this, &BrowserView::showContextMenu);
    ...
    connect(webView, &QWebEngineView::titleChanged, this, &BrowserView::titleChanged);
    connect(clearAllAction, &QAction::triggered, this, &BrowserView::clearAllStoredData);
    connect(page, &CustomWebPage::linkClicked, this, &BrowserView::handleLinkClicked);
...
void BrowserView::clearAllStoredData() {
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QMessageBox::information(this, "Cleared", "All browsing data has been cleared.");
...
QString BrowserView::currentUrl() const {
...
void BrowserView::clearData() {
    ...
    if (reply == QMessageBox::Yes) {
        ...
        profile->clearHttpCache();
        profile->clearAllVisitedLinks();
        QMessageBox::information(this, "Cleared", "Browsing data has been cleared.");
        webView->reload();
    }
}
...
QString BrowserView::sanitizeUrl(const QString &url) {
    ...
    if (!url.contains("://")) {
        ...
        if (url.contains('.')) {
            ...
            sanitized = "https://www.google.com/search?q=" + QUrl::toPercentEncoding(url);
        }
    }
    return sanitized;
}

void BrowserView::handleLinkClicked(const QUrl &url) {
    emit createTab(url);
}
