...
void Terminal::createToolBar() {
    ...
    // New Tab action
    QAction *newTabAction = toolbar->addAction(style()->standardIcon(QStyle::SP_FileIcon), "New Terminal");
    connect(newTabAction, &QAction::triggered, this, &Terminal::addNewTab);

    // Split actions
    QAction *splitHAction = toolbar->addAction(style()->standardIcon(QStyle::SP_ToolBarHorizontalExtensionButton), "Split Horizontal");
    connect(splitHAction, &QAction::triggered, this, &Terminal::splitHorizontally);

    QAction *splitVAction = toolbar->addAction(style()->standardIcon(QStyle::SP_ToolBarVerticalExtensionButton), "Split Vertical");
    connect(splitVAction, &QAction::triggered, this, &Terminal::splitVertically);

    // Close split action
    QAction *closeSplitAction = toolbar->addAction(style()->standardIcon(QStyle::SP_TitleBarCloseButton), "Close Split");
    connect(closeSplitAction, &QAction::triggered, this, &Terminal::closeCurrentSplit);
...
void Terminal::addNewTab() {
    ...
    int index = tabWidget->addTab(splitter, tr("Terminal %1").arg(tabWidget->count() + 1));
...
void Terminal::closeCurrentSplit() {
    ...
    if (!splitter || splitter->count() <= 1) return;
