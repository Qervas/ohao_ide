...
void CodeFolding::foldAll(QTextDocument *doc) {
  if (!doc)
    return;
...
void CodeFolding::paintFoldingMarkers(QPainter &painter,
                                      const QTextBlock &block,
                                      const QRectF &rect, bool folded,
                                      bool hovered, int lineNumberAreaWidth) {
  if (!isFoldable(block))
    return;

  int top = rect.top();
  qreal markerSize = hovered ? 16.0 : 12.0;
  qreal yOffset = (rect.height() - markerSize) / 2;
  ...
  qreal lineLength = markerSize * 0.7;
...
bool CodeFolding::isFoldMarkerUnderMouse(const QPoint &pos,
